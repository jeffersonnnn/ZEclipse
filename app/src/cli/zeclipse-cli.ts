...
async function main() {
  ...
  const zeclipseClient = new ZEclipseClient(connection, wallet, programId);
  ...
  switch (command) {
    case 'transfer':
      ...
      zeclipseClient.setEfficiencyInfoDisplay(Boolean(argv.efficiency));
      ...
      try {
        const signature = await zeclipseClient.executeAnonymousTransfer(
          amount, 
          recipient,
    ...
    default:
      ...
      console.log('  zeclipse transfer <betrag_in_sol> <empfänger> [--multi=addr1,addr2,addr3] - Führt einen anonymen Transfer durch');
      console.log('  zeclipse efficiency <amount_in_sol> [--recipients=n] - Shows cost efficiency information');
      console.log('  zeclipse balance - Zeigt das Wallet-Guthaben an');
      break;
  }
}
