...
pub fn convert_error(err: PurePoseidonError) -> anchor_lang::error::Error {
    ...
    match err {
        PurePoseidonError::HashingError(_) => error!(ZEclipseError::HashingError),
        PurePoseidonError::ValidationError(_) => error!(ZEclipseError::InvalidParameters),
        PurePoseidonError::ConversionError(_) => error!(ZEclipseError::HashingError),
    }
}
