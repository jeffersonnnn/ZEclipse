...
impl TransferConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        require!(self.num_hops > 0, ZEclipseError::InvalidConfig);
        require!(self.real_splits > 0, ZEclipseError::InvalidConfig);
        require!(self.fake_splits > 0, ZEclipseError::InvalidConfig);
        require!(self.fee_rate <= 10_000, ZEclipseError::InvalidConfig); // Max 100%
        require!(self.min_transfer_amount > 0, ZEclipseError::InvalidConfig);
        require!(
            self.max_transfer_amount > self.min_transfer_amount,
            ZEclipseError::InvalidConfig
        );
...
impl TransferState {
    ...
    pub fn increment_hops(&mut self, clock: &Clock) -> Result<()> {
        self.hops_completed = self.hops_completed.checked_add(1).ok_or(ZEclipseError::ArithmeticOverflow)?;
        self.updated_at = clock.unix_timestamp;
        Ok(())
    }
}
