...
/// Starting with version 3.0, the standalone `zeclipse_poseidon` package is used, which provides
...
extern crate hex;
use zeclipse_poseidon; // Independent Poseidon package
...
pub mod core {
    ...
    pub fn validate_parameters() -> std::result::Result<(), PoseidonError> {
        ...
        match zeclipse_poseidon::hash::validate_parameters() {
            Ok(_) => Ok(()),
    ...
    pub fn generate_hash(inputs: &[&[u8]]) -> std::result::Result<[u8; 32], PoseidonError> {
        zeclipse_poseidon::hash::generate_hash(inputs)
    ...
    pub fn batch_process(input_sets: &[Vec<&[u8]>]) -> std::result::Result<Vec<[u8; 32]>, PoseidonError> {
        zeclipse_poseidon::hash::batch_hash(input_sets)
...
pub mod anchor {
    ...
    use crate::errors::ZEclipseError;
    ...
    pub fn core_error_to_anchor_error(err: &core::PoseidonError) -> anchor_lang::error::Error {
        match err {
            core::PoseidonError::InvalidParameters => ZEclipseError::InvalidParameters.into(),
            core::PoseidonError::HashingError => ZEclipseError::HashingError.into(),
            core::PoseidonError::BatchError => ZEclipseError::HashingError.into(), // Use HashingError for batch errors as well
        }
    }
    ...
    pub fn poseidon_error_to_anchor_error(err: &super::PoseidonError) -> anchor_lang::error::Error {
        match err {
            super::PoseidonError::InvalidParameters => ZEclipseError::InvalidParameters.into(),
            super::PoseidonError::HashingError => ZEclipseError::HashingError.into(),
            super::PoseidonError::BatchError => ZEclipseError::HashingError.into(),
            super::PoseidonError::SerializationError => ZEclipseError::DeserializationError.into(),
        }
    }
    
    /// Converts Blackout-Poseidon errors to Anchor errors
    pub fn zeclipse_poseidon_error_to_anchor_error(err: &zeclipse_poseidon::PoseidonError) -> anchor_lang::error::Error {
        match err {
            zeclipse_poseidon::PoseidonError::ValidationError(_) => ZEclipseError::InvalidParameters.into(),
            zeclipse_poseidon::PoseidonError::HashingError(_) => ZEclipseError::HashingError.into(),
            zeclipse_poseidon::PoseidonError::ConversionError(_) => ZEclipseError::DeserializationError.into(),
...
pub fn generate_zk_hash(inputs: &[&[u8]]) -> anchor_lang::Result<[u8; 32]> {
    core::generate_hash(inputs)
        .map_err(|e| {
            ...
            crate::errors::ZEclipseError::HashingError.into()
        })
}
...
#[cfg(not(feature = "no-entrypoint"))]
pub fn poseidon_error_to_zeclipse_error(err: PoseidonError) -> anchor_lang::error::Error {
...
pub fn zeclipse_poseidon_error_to_error(err: zeclipse_poseidon::PoseidonError) -> anchor_lang::error::Error {
    anchor::zeclipse_poseidon_error_to_anchor_error(&err)
}
...
pub fn debug_hash_for_test(input: &[u8]) -> std::result::Result<[u8; 32], String> {
    zeclipse_poseidon::hash::debug_hash(input, "test")
...
mod tests {
    ...
    fn test_error_conversion() {
        ...
        let err = zeclipse_poseidon::PoseidonError::HashingError("Testfehler".to_string());
        let _anchor_err = zeclipse_poseidon_error_to_error(err);
