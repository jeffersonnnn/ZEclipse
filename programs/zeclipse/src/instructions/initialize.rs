...
pub fn initialize(
    ...
    if amount == 0 {
        ...
        return Err(ZEclipseError::InvalidAmount.into());
    }
    ...
    if ctx.accounts.payer.key() == ctx.accounts.primary_recipient.key() {
        ...
        return Err(ZEclipseError::InvalidRecipient.into());
    }
    
    // Use fixed ZEclipse configuration
    let config = BlackoutConfig::new();
    
    // Check challenge data
    if challenge == [0; 32] {
        msg!("Invalid challenge data");
        return Err(ZEclipseError::InvalidChallenge.into());
    }
    ...
    if !verify_range_proof(&range_proof, amount).map_err(|e| {
        ...
        ZEclipseError::InvalidRangeProof
    })? {
        transfer_ctx.error(&"Range proof verification failed", "Initialize");
        return Err(ZEclipseError::InvalidRangeProof.into());
    }
    ...
    let net_amount = amount.checked_sub(fee_amount).ok_or_else(|| {
        ...
        ZEclipseError::ArithmeticOverflow
    })?;
    ...
    if fee_amount < ctx.accounts.transfer_config.min_transfer_amount {
        ...
        transfer_ctx.error(&msg, "Initialize");
        return Err(ZEclipseError::InsufficientFunds.into());
    }
    ...
    **transfer_state = TransferState::new(
    ...
    transfer_state.initialize(
        ...
        net_amount,
        fee_amount,
        *ctx.bumps.get("transfer_state").ok_or(ZEclipseError::InvalidStateTransition)?,
        &clock,
    );
