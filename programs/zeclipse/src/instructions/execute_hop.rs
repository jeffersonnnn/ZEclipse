...
pub struct ExecuteHop<'info> {
    ...
    #[account(
        ...
        constraint = !transfer_state.refund_triggered @ ZEclipseError::TransferRefunded,
    )]
...
pub fn execute_hop(
    ...
    if transfer_state.current_hop != hop_index {
        return Err(ZEclipseError::InvalidHopIndex.into());
    }
    ...
    // Determine if this is the last hop (Hop #3 since we count from 0)
    let is_last_hop = hop_index == 3;
    ...
    let cu_limit_ix = ComputeBudgetInstruction::set_compute_unit_limit(cu_required);
    ...
    for i in 0..4 {
        ...
        let (split_pda, _split_bump) = derive_stealth_pda(
            ctx.program_id,
            &transfer_state.seed,
            hop_index,
            i,
        ...
        if ctx.accounts.split_pda.key() == split_pda {
            ...
            if available_lamports < split_amount {
                ...
                return Err(ZEclipseError::InsufficientLamports.into());
            }
            ...
            // Maximum cost efficiency: Close all temporary accounts immediately after use
            // and recover all remaining lamports (even on last hop)
            {
                msg!("Cost optimization: Returning rent lamports from split PDA {}", i);
                // Transfer all remaining lamports back to the transfer_state
                let split_account_info = ctx.accounts.split_pda.to_account_info();
                let rent_exempt_minimum = Rent::get()?.minimum_balance(0);
                
                // Recover anything above the rent exemption
                if split_account_info.lamports() > rent_exempt_minimum {
                    ...
                    **transfer_state.to_account_info().lamports.borrow_mut() += recoverable_amount;
                    msg!("Recovered {} lamports from temporary account", recoverable_amount);
                }
            }
