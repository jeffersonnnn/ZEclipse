...
/// Converts an error into a ZEclipseError with context
pub trait IntoZEclipseError<T> {
    /// Converts the error into a ZEclipseError with the given context
    fn zeclipse_err(self, context: &str) -> Result<T, ErrorContext>;
}

impl<T, E> IntoZEclipseError<T> for Result<T, E>
where
    E: StdError + Send + Sync + 'static,
{
    fn zeclipse_err(self, context: &str) -> Result<T, ErrorContext> {
